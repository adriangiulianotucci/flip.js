...
var flip = (function () {
  var flip = {},
    ...
    opera = window.opera,
    vendor = webkit ? "webkit" : firefox ? "" : ie ? "ms" : opera ? "O" : "",
  ...
  getAngle = function () {
    ...
    if (transitionForward) {
      ...
      angle =
        (180 * deltaX) /
        Math.min(windowWidth / 2, draggableControl.startXY.x * 0.9);
      ...
      angle =
        ...
        Math.min(
          windowWidth / 2,
          (windowWidth - draggableControl.startXY.x) * 0.9
        );
  ...
  dragListener = {
    ...
    reset: function () {
      ...
      if (angle === 0) {
        ...
        rightFlip.style[transformKey] =
          "translateX( 0 ) perspective( " +
          3000 +
          perspectiveUnits +
          ") rotateY( -0deg )";
        rightFlip.style[transitionKey] =
          transformProp + " " + (500 * angle) / 90 + "ms ease-in";
        rightFlip.addEventListener(
          transitionEndEventName,
          dropRightHandler,
          false
        );
      } else if (leftFlip) {
        leftFlip.style[transformKey] =
          "translateX( 0 ) perspective( " +
          3000 +
          perspectiveUnits +
          ") rotateY( -360deg )";
        leftFlip.style[transitionKey] =
          transformProp + " " + (500 * (180 - angle)) / 90 + "ms ease-in";
        leftFlip.addEventListener(
          transitionEndEventName,
          dropLeftHandler,
          false
        );
  ...
  flip.pop = function () {
    ...
    if (topPage !== frontPage) {
      ...
      setTimeout(function () {
        var baseTransform =
          "translateX(0px) perspective(3000" + perspectiveUnits + ") ";
  ...
  flip.push = function (newPageControl) {
    ...
    if (!frontPage) {
      ...
      setTimeout(function () {
        var baseTransform =
          "translateX( 0 ) perspective(3000" + perspectiveUnits + ") ";
...
flip.basic = function (bookElement) {
  ...
  bookElement =
    typeof bookElement === "string"
      ? document.getElementById(bookElement)
      : bookElement;
...
flip.page = (function () {
  ...
  return {
    ...
    create: function (pageContentElem, listener) {
      ...
      draggable.listener = listener;
      draggable.html = flip.createElem("div", {
